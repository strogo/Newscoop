...
function smarty_block_blogentry_form($p_params, $p_content, &$p_smarty, &$p_repeat)
{
    ...
    if (isset($p_params['template'])) {
        ...
        if (!$template->defined) {
            ...
            . '" specified in the blog entry form');
            return false;
        }
    ...
    $html .= "<input type=\"submit\" name=\"f_submit_blogentry\" "
    ."id=\"blogentry_submit\" value=\""
    .smarty_function_escape_special_chars($p_params['submit_button'])
    ."\" " . $p_params['button_html_code'] . " />\n";
    if (isset($p_params['preview_button']) && !empty($p_params['preview_button'])) {
        $html .= "<input type=\"submit\" name=\"f_preview_blogentry\" "
        ."id=\"blogentry_preview\" value=\""
