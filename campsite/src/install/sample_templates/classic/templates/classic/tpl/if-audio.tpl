...
{{ if $counter == 1 }}
    <div class="audio-entity"> 
     <h4>{{ if $sf->language->name == "English" }}Listen to audio attachments{{ else }}Escucha a los accesorios de audio{{ /if }}</h4> 
...
<script type="text/javascript">
  var options = {};
  options.mediaPath = "http://{{ $sf->publication->site }}{{ uri options="articleattachment" }}";
  ...
  swfobject.embedSWF("/templates/classic/tpl/player/player.swf", "player-holder", "250", "21", "9.0.0",false, options, {}, {});
       ...
       <div class="audio-attachment-description">{{ $sf->attachment->description }}</div> 
