...
function camp_get_permission_list()
{
	...
	$administrative_group = array('ChangeSystemPreferences'=>getGS('User may change system preferences'),
    ...
    foreach (CampPlugin::GetPluginInfos() as $info) {
        if (CampPlugin::IsPluginEnabled($info['name']) && is_array($info['permissions'])) {
