<?
regGS("Publication<BR><SMALL>(click to see sections)</SMALL>","Публикация<BR><SMALL>(Щелкните для просмотра секций)</SMALL>:ru");
regGS("Left to pay","Осталось оплатить:ru");
regGS("Active","Активно:ru");
regGS("Delete subscriptions to $1","Удалить подписку на $1:ru");
regGS("No subscriptions.","Нет подписок.:ru");
regGS("Are you sure you want to delete the subscription to the publication $1?","Вы уверены, что хотите удалить подписку на публикацию $1?:ru");
regGS("The subscription has been deleted.","Подписка была удалена:ru");
regGS("The subscription could not be deleted.","Невозможно удалить подписку.:ru");
regGS("Start","Начало:ru");
regGS("Days","Дней:ru");
regGS("(YYYY-MM-DD)","(ГГГГ-ММ-ДД):ru");
regGS("Add sections now","Добавить секцию сейчас:ru");
regGS("Add sections later","Добавить секцию позже:ru");
regGS("The sections were added successfully.","Секции были успешно добавлены.:ru");
regGS("The subscription has been added successfully.","Подписка была успешно добавлена.:ru");
regGS("The subscription could not be added.","Невозможно добавить подписку.:ru");
regGS("Please check if there isn't another subscription to the same publication.","Пожалуйста проверьте, что нет другой подписки на эту публикацию.:ru");
regGS("The sections could not be added successfully. Some of them were already added !","Секции не могут быть добавлены. Некоторые из них уже были добавлены !:ru");
regGS("Change subscription status","Изменить статус подписки:ru");
regGS("ou do not have the right to change subscriptions status.","Вы не имеете права на изменение статуса подписок.:ru");
regGS("Are you sure you want to deactivate the subscription?","Вы уверены, что хотите деактивировать подписку?:ru");
regGS("Are you sure you want to activate the subscription?","Вы уверены, что хотите активировать подписку?:ru");
regGS("Changing subscription status","Изменение статуса подписки:ru");
regGS("The subscription has been activated.","Подписка активирована.:ru");
regGS("The subscription has been deactivated.","Подписка деактивирована.:ru");
regGS("Subscription status could not be changed.","Невозможно изменить статус подписки.:ru");
regGS("Update payement","Обновление оплаты:ru");
regGS("The subscription payement was updated.","Оплата подписки была обновлена.:ru");
regGS("Subscription payement could not be changed.","Невозможно изменить оплату подписки.:ru");

?>