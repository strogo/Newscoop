...
{{ if !$sf->url->is_valid }}
  <h3>{{ if $sf->language->name == "English" }}The requested page was not found.{{ else }}La página solicitada no fue encontrada.{{ /if }}</h3>
  {{ set_language name=`$sf->publication->default_language->english_name` }}
  {{ set_current_issue }}
{{ else }}
...
<h3>{{ if $sf->language->name == "English" }}Other articles{{ else }}Otros artículos{{ /if }}</h3>
