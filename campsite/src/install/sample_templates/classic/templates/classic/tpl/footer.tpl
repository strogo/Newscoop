...
<div id="block-articles-mostpopular-item" class="block-articles-mostpopular-item-{{ $sf->section->number }}">
<div id="block-articles-mostpopular-iteminner" class="block-articles-mostpopular-iteminner-{{ $sf->section->number }}">
<a href="{{ uri }}" class="block-mostpopular-link">
<div id="block-mostpopular-section" class="block-mostpopular-section-{{ $sf->section->number }}">{{ $sf->section->name }}</div>
<div id="block-mostpopular-title">{{ $sf->article->name }}</div>
<div id="block-mostpopular-deck">{{ $sf->article->deck }}</div>
<!--div class="block-mostpopular-link" id="block-mostpopular-counter">({{ $sf->article->reads }} reads)</div-->
</a>
...
<center>
          <a href="{{ uri options="issue" }}">{{ if $sf->language->name == "English" }}Home{{ else }}Portada{{ /if }}</a>
      {{ list_sections name="sections" constraints="number greater_equal 10 number smaller_equal 120" }}
          <a href="{{ uri }}">{{ $sf->section->name }}</a>
      {{ /list_sections }}
</center>
</div><!-- id="footerlinksinner" -->
</div><!-- id="footerlinksinner" -->
